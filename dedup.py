...
def main():
    ...
    parser_gen.add_argument("-o", "--output", default="./dedup.txt", help="generate dedup info in this file")
    parser_gen.add_argument("-v", "--verbose", action="store_true", help="Print verbose info while generating deduplication list")
    ...
    if args.command == "generate":
        handle_generate(args.directories, args.output, args.verbose)
...
def handle_generate(directories, output="./dedup.txt", verbose=False):
    dedup = {}
    duplicates = {}
    ...
    if verbose:
        orig_len = len(duplicates)
        print(f"Found {orig_len} hashes")
    ...
    with open(output, "w") as output:
        ...
        if verbose:
            print("Wrote deduplication info to", output.name)
